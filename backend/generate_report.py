...
if not all([API_KEY, ENDPOINT, DEPLOYMENT, API_VERSION]):
    raise RuntimeError("Faltan variables de Azure OpenAI en .env")
...
def generar_informe(perfil: str) -> str:
    """
    Llama a Azure OpenAI (GPT) para generar el texto completo del informe,
    con la estructura solicitada en el prompt.
    """
    prompt = f"""
Eres un orientador laboral experto en empleabilidad, neurodiversidad y discapacidad intelectual.
Tu tarea es analizar el perfil que te damos a continuación y generar un INFORME PROFESIONAL DE EMPLEABILIDAD,
estructurado y redactado en lenguaje claro, directo, accesible y humano.
...
"""
    ...
    respuesta = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Eres un psicólogo laboral experto en orientación, neurodiversidad y accesibilidad cognitiva."
            },
            {
                "role": "user",
                "content": prompt
