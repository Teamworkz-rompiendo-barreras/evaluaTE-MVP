...
function getImagePath(imageName) {
    ...
    const possiblePaths = [
        (0, path_1.join)(__dirname, '../assets', imageName),
        (0, path_1.join)(__dirname, '../../src/assets', imageName),
        (0, path_1.join)(process.cwd(), 'src/assets', imageName),
        (0, path_1.join)(process.cwd(), 'dist/src/assets', imageName)
    ];
...
async function safeLoadImage(path, label) {
    try {
        return await (0, canvas_1.loadImage)(path);
    }
    catch (err) {
...
const createPdf = async (data) => {
    ...
    const canvas = (0, canvas_1.createCanvas)(800, 1200, 'pdf');
    ...
    if (Array.isArray(jobPreferences.areas)) {
        jobPreferences.areas.forEach((area, index) => {
