...
function DarkZoomWidget() {
  ...
  return (
    <div className="fixed bottom-[10px] right-[10px] z-50 flex flex-wrap items-center justify-end gap-[4px] bg-white/90 dark:bg-gray-800/90 border border-gray-200 dark:border-gray-700 rounded-full px-[6px] py-[5px] shadow print-hidden max-w-[180px] origin-bottom-right">
      <button onClick={() => setDark(d => !d)} className="px-[8px] py-[5px] rounded-full text-[11px] bg-gray-200 dark:bg-gray-700 hover:opacity-90 whitespace-nowrap">{dark ? 'Modo claro' : 'Modo oscuro'}</button>
      <div className="flex items-center gap-[3px]">
        <button onClick={() => setZoom(z => Math.max(80, z - 10))} className="w-[24px] h-[24px] rounded-full text-[11px] bg-gray-200 dark:bg-gray-700 flex items-center justify-center">-</button>
        <span className="text-[11px] w-[34px] text-center">{zoom}%</span>
        <button onClick={() => setZoom(z => Math.min(160, z + 10))} className="w-[24px] h-[24px] rounded-full text-[11px] bg-gray-200 dark:bg-gray-700 flex items-center justify-center">+</button>
      </div>
    </div>
  );
}
