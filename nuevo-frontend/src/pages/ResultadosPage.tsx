...
export default function ResultadosPage() {
  ...
  // FIX TÉCNICO: Eliminada la mutabilidad (delete). Uso de desestructuración estricta tipada.
  const displayPersonalData = useMemo(() => {
    if (!reportData?.datos_personales) return {};
    
    // Extraemos 'nombre' y 'Nombre' para descartarlos de forma inmutable, guardando el resto en 'restDatos'
    const { nombre, Nombre, ...restDatos } = reportData.datos_personales;
    
    // Creamos un nuevo objeto tipado explícitamente para satisfacer al compilador TS
